...
def add_cats(connection, amount):
    ...
    for i in range (amount):
        connection2 = PrestaShopWebServiceDict('http://localhost/api/categories/'+ str(i+1), API_KEY)
        classes = connection2.get("category")["category"]["name"]["language"]["value"]
        cat_tab[classes] = i+1
    ...
    with open("categories-temp.xml", "r", encoding='utf-8') as cat_file:
        ...
        for cat in cat_list:           
            ...
            name = "<![CDATA[3]]>" #name
            cat_output = cat_temp_output.replace(parent,"<![CDATA["+ str(cat_tab["Home"]) +"]]>")
...
def add_subs(connection, main_cat_len):
    ...
    with open("categories-temp.xml", "r") as cat_file:
        ...
        for sub in sub_list:
            ...
            parent = "<![CDATA[2]]>" #parent
            name = "<![CDATA[3]]>" #name
            cat_output = cat_temp_output.replace(parent,"<![CDATA["+ str(cat_tab[sub[0]]) +"]]>")
